...
global.myJSONtopbar = "";
global.myJSONbotbar = "";
function printerWebScraping(){

    const puppeteer = require('puppeteer');
    

    var printerHostname = "br-h310-prn2";
    (async ()=> {
...
function intialize(){
    ...
    function countdown(){
        if (timeleft == -1) {
            var lineReader = require('readline').createInterface({input: require('fs').createReadStream('Printers2.txt')});
            lineReader.on('line',function(line){
            const puppeteer = require('puppeteer');
    
            
            var printerHostname = line;
            console.log(printerHostname);
            (async ()=> {
